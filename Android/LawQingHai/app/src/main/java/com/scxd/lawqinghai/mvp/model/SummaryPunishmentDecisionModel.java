...
public class SummaryPunishmentDecisionModel implements SummaryPunishmentDecisionModelImp {
     ...
     */
    @Override
    public void backSummaryPunishmentDecision(String user, String bmbh, String starTime, String endTime, int page, final MVPCallBack mvpCallBack) {

        SummaryPunishmentDecisionReqBean object = new SummaryPunishmentDecisionReqBean(starTime, endTime,
                user, bmbh, page, 10);


        NetWorking.requstJsonNetData("Q17", Common.QUERY, object, new DataCallback() {
            @Override
            ...
            public void onResponse(String response, int id) {
                try {
                    ...
                    if (Common.isDataTest) {
                        response = AssestToString.xmltoString(MyApplication.appContext, "dataTest/Q17.json");
                    }

                    SummaryPunishmentDecisionResBean jsonObj = JSON.parseObject(response, SummaryPunishmentDecisionResBean.class);
                    ...
                    LogUtil.open().appendMethodB("返回Q17Exception:" + e.getMessage() + "--" + e.toString() + "\n");

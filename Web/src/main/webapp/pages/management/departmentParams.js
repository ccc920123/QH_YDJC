...
function loadParam() {
    doGet('params/management/department/params', {
        bmbh: config.bmbh,
        csmc: $('#params').val(),
        page: config.pageNo,
...
function loadParamTable(data) {
    ...
    var bianji = justPressionBy("1129");
    var tbody = '';
...
function editParam(span) {
    ...
    $('#confirmSubmit').bind('click', function () {
        //当前配置是继承
        if ("1" == sfjc) {
            var jc = $("input[name='status']:checked").val();
            //当前选择不继承父级配置则新增
            if ("0" == jc) {
                addParams();
            }else {
    ...
    $('#extends').unbind('click');
    $('#extends').bind('click', function () {
        //判断本来是继承上级或者是当前的bmbh为为总队直接关闭对话框
        if ("1" == sfjc) {
            layer.closeAll();
        } else {
            if ("630000000000" == $('#bmdm').val()) {
                layer.closeAll();
            } else {
                deleteParams(id);
            }
...
function addParams() {
    doPost("params/management/department/addParams",JSON.stringify( {
        id:'',
        bmbh: config.bmbh,
        bmmc: '',
        sfjc:'',
...
function deleteParams(id) {
    doPost("params/management/department/deleteParams",JSON.stringify( {
        id: id
...
function updateParam(id) {
    ...
    doPost('params/management/department/updateParams',JSON.stringify( {

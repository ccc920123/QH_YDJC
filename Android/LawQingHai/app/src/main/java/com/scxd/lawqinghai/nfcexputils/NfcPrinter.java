...
import com.scxd.lawqinghai.nfcexputils.Nfc_const.R_W_Methods;
import com.scxd.lawqinghai.nfcexputils.Nfc_const.Task_Result;
import com.scxd.lawqinghai.activity.PrintImageActivity;
...
//        }
    ...
    public boolean tag_connect(Tag tag) {
        ...
        try {
            ...
            frame = new Nfc_frame(reader, R_W_Methods.Fast_Mode);
     ...
     * @return
     */
    private Task_Result nfcConnet(final int timeout) {
        ...
        try {
            while (true) {
                ...
                if (((System.currentTimeMillis() - start) > timeout)) {
                    return Task_Result.CONNET_TIMEOUT;
                }
                Thread.sleep(5);
            }
        } catch (Exception e) {
            return Task_Result.EXCEPTION;
        }
    ...
    private void sendMessageToUI(String str) {
        PrintImageActivity.setMessageCallback(str);
    }

    /**
     * 启动异步线程发送数据
     */
    private class PrintTask extends AsyncTask<Integer, Integer, Task_Result> {
         ...
         */
        @Override
        protected Task_Result doInBackground(Integer... param) {
            byte id = 0;
            if (exitTask) {
                return Task_Result.CANCEL;
            }
            try {
                int timeout = param[0];
                Task_Result result = nfcConnet(timeout);
                if (result != Task_Result.SUCCESS) {
                    return result;
                }
                if (m_dataBuffer == null)
                    return Task_Result.WAIT_DATA_TIMEOUT;
                 //开始向Exp342设备发送数据
                if (!printerSendData(m_dataBuffer, 0, m_dataBuffer.length)) {
                    return Task_Result.SendFail;
                }
                return Task_Result.SUCCESS;
            } catch (Exception e) {
                return Task_Result.EXCEPTION;
         ...
         */
        @Override
        protected void onPostExecute(Task_Result result) {
            if (result == Task_Result.CANCEL) {
                sendMessageToUI("取消任务");
            } else if (result == Task_Result.SUCCESS) {
                sendMessageToUI("1");
            } else if (result == Task_Result.EXCEPTION) {
                sendMessageToUI("发送异常");
            } else if (result == Task_Result.WAIT_DATA_TIMEOUT) {
                sendMessageToUI("等待数据超时");
            } else if (result == Task_Result.CONNET_TIMEOUT) {
                sendMessageToUI("连接超时");
            } else if (result == Task_Result.SendFail) {
